    ...
    public static class NiUtils
    {
        ...
        public static void VertexLaserDraw(Texture2D collitex, Texture2D colorTex, Texture2D shapeTex, Texture2D maskTex, Vector2[] veclaser,Vector2 startpos,float rotation,float interval,int collitexsize, float size = 1f)
        {
            ...
            if (bars.Count > 2)
            {
                ...
                // 把变换和所需信息丢给shader
                AssetHelper.SetColor.Parameters["uTransform"].SetValue(model * projection);
                AssetHelper.SetColor.Parameters["uTime"].SetValue(-(float)Main.time * 0.03f);
                ...
                AssetHelper.SetColor.CurrentTechnique.Passes[0].Apply();
            ...
            for (int i = 0; i < Main.maxDustToDraw; i++)
            {
                ...
                if (new Rectangle((int)dust.position.X, (int)dust.position.Y, 4, 4).Intersects(rectangle))
                {
                    ...
                    for (int n = 0; n < traillength; n++)
                    {
                        ...
                        sb.Draw(AssetHelper.ElecCyan_Dust, pos - Main.screenPosition, dust.frame, poscolor, dust.rotation, new Vector2(4f, 4f), scale, SpriteEffects.None, 0f);
                    }
